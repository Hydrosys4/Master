...
def cyclereset(element):
	...
	AUTO_data[element]={"lasteventtime":datetime.utcnow() - timedelta(minutes=waitingtime),"status":"ok","actionvalue":0, "alertcounter":0, "infocounter":0}
...
def interruptexecute(refsensor,element):		
	...
	if workmode=="Pre-emptive Blocking":
		...
		if timeok:
			...
			CheckActivateNotify(element,sensor,preemptiontime,actuatoroutput,actionmodeafterfirst,actuatoroutputfollowup,mailtype)
...
def CheckActivateNotify(element,sensor,preemptiontime,actuatoroutput,actionmodeafterfirst,actuatoroutputfollowup,mailtype):
